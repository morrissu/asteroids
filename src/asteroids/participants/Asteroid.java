...
public class Asteroid extends Participant implements ShipDestroyer
{
    ...
    public void collidedWith (Participant p)
    {
        if (p instanceof AsteroidDestroyer)
        {
            ...
            int size = getSize() - 1;
            if (size >= 0)
            {// -M & VinC; assigns appropriate speed to asteroids of different sizes
                controller.addParticipant(new Asteroid(RANDOM.nextInt(4), size, getX(), getY(), (3 + RANDOM.nextInt(3 - size)), controller));
                controller.addParticipant(new Asteroid(RANDOM.nextInt(4), size, getX(), getY(), (3 + RANDOM.nextInt(3 - size)), controller));
                if(enhancedMode){
                	if(RANDOM.nextFloat()<0.3f){
                		controller.addParticipant(new VinC_Loot(getX(), getY(), controller));
                	}
                }
            }
